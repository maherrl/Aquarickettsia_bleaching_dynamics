...
getwd()
load("./data/ps_rar8692.RData")
ps
```

### Calculating alpha diversity statistics
Initialize matrices to store obsness and simpson estimates
```{r init}
nsamp = nsamples(ps)
...
# Calculate chao1
hao <- as.numeric(as.matrix(subset(estimate_richness(ps, measures = "Chao1"), select = c(1))))
chao1[ ,] <- chao
...
```{r compile}
alpha <- cbind(observed, chao1,simpson,shannon, faithPD)
...
alphadiv <- alphadiv[,-6]
head(alphadiv)
#write.csv(alphadiv, file = "../data/alphadiv8692.csv")
```